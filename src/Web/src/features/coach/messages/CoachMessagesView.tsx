...
import { initialCoachMessagesData } from './mockData';
import { CoachConversation, CoachMessageItem, CoachMessagesViewModel } from './types';

export interface CoachMessagesViewProps {
  initialData?: CoachMessagesViewModel;
  isLoading?: boolean;
  errorMessage?: string;
  onRetry?: () => void;
}

export const CoachMessagesView: React.FC<CoachMessagesViewProps> = ({
  initialData = initialCoachMessagesData,
  isLoading = false,
  errorMessage,
  onRetry,
}) => {
  ...
  const [conversations, setConversations] = useState<CoachConversation[]>(initialData.conversations);
    ...
    <div className="max-w-6xl mx-auto select-none space-y-4 sm:space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-neutral-400 mt-0.5">
          Atanmış öğrencileriniz ile doğrudan iletişim ve soru-cevap akışı
        </p>
      </div>
        ...
        >
          {activeConversation ? (
            <>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-neutral-400">
                      {activeConversation.studentGrade}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 space-y-3 overflow-y-auto py-2 pr-1 max-h-[360px]">
                {activeConversation.messages.map((msg) => {
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={cn('flex flex-col', isCoach ? 'items-end' : 'items-start')}
                    >
                      <div
                        className={cn(
              ...
              <form onSubmit={handleSendMessage} className="pt-3 border-t border-neutral-100 flex items-center gap-2">
                <input
                  ...
                  placeholder={`${activeConversation.studentName} adlı öğrenciye mesaj yazın...`}
            ...
            <EmptyState
              ...
              description="Mesajlaşmayı görüntülemek için sol menüden bir öğrenci seçin."
            />
          )}
        </div>
      </div>
